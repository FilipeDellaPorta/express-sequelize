        ...
        try {
            const listaMatriculasPorEstudante = await matriculaServices.pegaEContaRegistros(
            {
                where: {
                 ...
                 limit: 2, //limit faz páginação
                 order: [['id', 'DESC']] //ASC de ascendente e DESC de descendente
                
            })
            return res.status(200).json(listaMatriculasPorEstudante) //se colocar listaMatriculasPorEstudante.count retorna somente o valor do count
        } catch (erro) {
